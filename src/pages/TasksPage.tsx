...
const TasksPage = () => {
  ...
  const [newTask, setNewTask] = useState({
    ...
    jungleId: jungles[0]?.id || '', alarmEnabled: false, alarmTime: '09:00', ringtone: 'default',
  });
  ...
  const handleAddTask = () => {
    ...
    setNewTask({ title: '', type: 'daily', dueDate: undefined, dueTime: '23:59', jungleId: jungles[0]?.id || '', alarmEnabled: false, alarmTime: '09:00', ringtone: 'default' });
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <main className="px-4 py-6 max-w-lg mx-auto space-y-6">
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          <DialogTrigger asChild><Button className="w-full gap-2 bg-accent hover:bg-accent/80"><Plus size={20} /> Add New Goal</Button></DialogTrigger>
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="glass-panel rounded-xl p-3 border border-accent/20">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2"><Bell className="w-4 h-4 text-accent" /><span className="text-sm font-medium">Reminder Alarm</span></div>
          ...
          {pendingTasks.length === 0 ? (
            ...
            <div className="space-y-2">
              {pendingTasks.map((task) => {
                ...
                return (
                  <div key={task.id} className={cn("task-item group", overdue && "border-destructive/50 bg-destructive/10 animate-pulse")}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2"><p className="text-sm truncate">{task.title}</p>{overdue && <AlertTriangle className="w-4 h-4 text-destructive shrink-0" />}</div>
