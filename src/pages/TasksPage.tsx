...
import { Plus, Trash2, Calendar, Clock, Bell, AlertTriangle, ChevronDown, ChevronUp, Check, X } from 'lucide-react';
...
const TasksPage = () => {
  ...
  const quickAdd = async () => {
    ...
    setQuickTitle(''); setQuickReminder('');
  };
              ...
              {monthDays.map(d => {
                ...
                return (
                  <button key={d.toISOString()} onClick={() => setSelectedDay(d)} className={cn(
                    ...
                    {items.length > 0 && <span className="w-1 h-1 rounded-full bg-primary" />}
                  </button>
                );
              })}
            </div>
